    ...
    for line in open(x, "rb"):
        ...
        if line[:4] == "ATOM":
            ...
            if ch == chain or chain is None:
                atom = line[12: 12 + 4].strip()  # atom type
                resi = line[17: 17 + 3]  # aa name
                resn = line[22: 22 + 5].strip()  # aa position
                x, y, z = [float(line[i: (i + 8)])
                           for i in [30, 38, 46]]  # 这个原子的坐标
    ...
    for resn in range(min_resn, max_resn + 1):
        ...
        if resn in xyz:
            for k in sorted(xyz[resn]):
                for atom in atoms:  # 只取这三种原子
                    if atom in xyz[resn][k]:
                        xyz_.append(xyz[resn][k][atom])  # 对应的原子坐标
                    else:
